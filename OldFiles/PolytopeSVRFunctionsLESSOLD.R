...
DoMiss = function(dat, missingY, missingProb){
	...
	stopifnot(is.numeric(missingProb) & missingProb>=0 & missingProb<1)
	...
	for(i in 1:n){
		dat[i, sample(totPotentialMissVar, round(totPotentialMissVar*missingProb))] = NA # each row independently of the other has a certain proportion of missing columns
	}
	return(dat)	
}

DoMissFold = function(doDataSplitOut, missingY, missingProb){
	...
	#  note this applies the missing process on training/testing independently 
		doDataSplitDoMissOut[[i]]$inDat$missing = DoMiss(doDataSplitOut[[i]]$inDat$original, missingY, missingProb)
		doDataSplitDoMissOut[[i]]$outDat$missing = DoMiss(doDataSplitOut[[i]]$outDat$original, missingY, missingProb) 
	}
	return(doDataSplitDoMissOut)
}

#doMissFoldOut = DoMissFold(doDataSplitOut, missingY=T, missingProb = .3)
...
DoMultipleImputation = function(missDat, method, numImput, maxIter, extraPossiblyMissDat=NULL){
	...
	imputDatList = list()
	for(j in 1:numImput){
		imputDatList[[j]] = as.matrix(complete(multiImputDat, j)[1:n,])
...
DoPolyList = function(missDat, imputDatList, medianImputDat, pcProp, scaleData, maxUncertainDims, doMedian){
	...
	for(i in 1:n){
		...
		if(any(is.na(currPoint))){
			...
			if(!all(apply(singleMissDatMultImput, 2, sd)==0)){
				...
				if(scaleData)
					currPointConstantImput = ScaleCenter(currPointConstantImput, scaleInfo$mean, scaleInfo$std)
...
DoPolyListFolds = function(doMultipleImputationFoldsOut, pcProp, scaleData, maxUncertainDims, doMedian){
	...
	for(i in 1:length(doMultipleImputationFoldsOut)){
		...
		polyListFolds[[i]]$polyList = DoPolyList(missDat=doMultipleImputationFoldsOut[[i]]$inDat$missing, 
			...
			pcProp=pcProp, scaleData=scaleData, maxUncertainDims=maxUncertainDims, doMedian=doMedian)
...
DoError = function(predY, trueY, missingDatOutLogical){
	...
	errList$quantNineAe = QuantNineAE(predY, trueY)
	errList$quantEightAE = QuantEightAE(predY, trueY)
	errList$quantSevenAE = QuantSevenAE(predY, trueY)

	errList$rmseUncert = MAE(predY, trueY, missingDatOutLogical)	
	errList$rmseUncert = RMSE(predY, trueY, missingDatOutLogical)
	errList$MaxaeUncert = MaxAE(predY, trueY, missingDatOutLogical)
	errList$quantNineAeUncert = QuantNineAE(predY, trueY, missingDatOutLogical)
	errList$quantEightAeUncert = QuantEightAE(predY, trueY, missingDatOutLogical)
	errList$quantSevenAeUncert = QuantSevenAE(predY, trueY, missingDatOutLogical)
	...
	errList$quantNineAeCert = QuantNineAE(predY, trueY, !missingDatOutLogical)
	errList$quantEightAeCert = QuantEightAE(predY, trueY, !missingDatOutLogical)
	errList$quantSevenAeCert = QuantSevenAE(predY, trueY, !missingDatOutLogical)
